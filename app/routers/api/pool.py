...
def _pool_to_dict(pool: BudgetPool) -> dict:
    return {
        ...
        "available": round(max(0.0, pool.total_funded - pool.total_consumed), 4),
        "used_quota": round(pool.total_allocated, 4),
        "total_quota": round(pool.total_funded, 4),
        "allocated": round(pool.total_allocated, 4),
